...
print("🚀 [INFO] Memulai Sistem Gesture Control (HANDS MODE) + DATA LOGGING...")
...
# ========================================================
# --- PERUBAHAN UTAMA: GANTI HOLISTIC KE HANDS ---
# ========================================================
mp_hands = mp.solutions.hands # Pakai Hands, bukan Holistic
...
# Fungsi ekstrak keypoints disesuaikan untuk output mp_hands
def extract_keypoints(results):
    # Cek apakah ada tangan terdeteksi
    if results.multi_hand_landmarks:
        # Ambil tangan pertama yang terdeteksi (karena max_num_hands=1)
        hand_landmarks = results.multi_hand_landmarks[0]
        rh = np.array([[res.x, res.y, res.z] for res in hand_landmarks.landmark]).flatten()
    else:
        # Jika tidak ada tangan, return array 0 sepanjang 21 titik x 3 dimensi
        rh = np.zeros(21*3)
    return rh

# --- LOAD MODEL TFLITE ---
TFLITE_MODEL_PATH = 'model.tflite'
interpreter = tf.lite.Interpreter(model_path=TFLITE_MODEL_PATH, num_threads=4) # Optimasi: Pakai 4 thread
...
# Opsional: Turunkan resolusi untuk FPS lebih tinggi (jika perlu)
...
with mp_hands.Hands(
    ...
    while cap.isOpened():
        ...
        # ==========================================
        # LOGIKA STATE MACHINE (SAMA PERSIS)
        # ==========================================
