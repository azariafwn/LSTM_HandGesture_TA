...
export default function Dashboard({ auth, logs }) {
    ...
    // Default true (Dark Mode), bisa diganti false jika ingin default Light Mode
    const [darkMode, setDarkMode] = useState(true);

    // Helper untuk format jam yang aman (Fix Timestamp Hilang)
    const formatTime = (timestamp) => {
        try {
            const date = new Date(timestamp);
            // Ambil jam:menit:detik (Format 24 Jam)
    ...
    // --- LOGIKA WARNA DINAMIS (THEME CONFIG) ---
    const theme = {
    ...
    // Komponen Kartu (Updated Font Size & Theme Logic)
    const DeviceCard = ({ name, status }) => (
        <div className={`relative group p-5 rounded-xl border transition-all duration-300 
            ...
            <div className={`absolute top-3 right-3 w-3 h-3 rounded-full ${status === 'ON' ? 'bg-cyan-400 animate-pulse' : 'bg-slate-500'}`}></div>

            <div className={`font-mono text-sm tracking-widest mb-2 font-bold ${status === 'ON' ? (darkMode ? 'text-cyan-100' : 'text-blue-900') : (darkMode ? 'text-slate-500' : 'text-gray-500')}`}>
                {name.toUpperCase()}
            </div>
            
            <div className={`text-2xl font-bold font-mono tracking-tighter ${status === 'ON' ? (darkMode ? 'text-cyan-400' : 'text-blue-600') : (darkMode ? 'text-slate-600' : 'text-gray-400')}`}>
    ...
    return (
        ...
        >
            ...
            {/* Main Wrapper dengan Transisi Warna */}
            <div className={`min-h-screen font-mono selection:bg-cyan-500 selection:text-black transition-colors duration-500 ${theme.bg} ${theme.text}`}>
                ...
                {/* Custom Header */}
                <div className={`max-w-7xl mx-auto px-6 pt-10 pb-6 flex justify-between items-end border-b ${theme.headerBorder} mb-8`}>
                    <div>
                        <h1 className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500 tracking-tighter">
                            SYSTEM_DASHBOARD
                        </h1>
                        <p className={`text-sm mt-1 tracking-widest ${darkMode ? 'text-slate-500' : 'text-gray-500'}`}>SECURE CONNECTION ESTABLISHED</p>
                    </div>
                    <div className="flex items-center gap-3">
                         <button 
                            onClick={() => setDarkMode(!darkMode)}
                            className={`px-4 py-2 rounded-lg border transition text-xs font-bold tracking-wider
                                ${darkMode 
                ...
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8 pb-12">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                        ...
                        <div className="lg:col-span-1 relative group">
                            {/* Sudut Dekoratif (Hanya muncul di Dark Mode) */}
                            {darkMode && (
                                <>
                            ...
                            <div className={`bg-black rounded-lg overflow-hidden shadow-2xl border relative h-64 lg:h-full min-h-[250px] flex items-center justify-center ${theme.cardBorder}`}>
                                {/* Efek Scanline (Dark Mode Only) */}
                        ...
                        <div className="lg:col-span-2 flex flex-col gap-4">
                            <div className={`${theme.cardBg} backdrop-blur border ${theme.cardBorder} p-5 rounded-xl flex justify-between items-center shadow-sm`}>
                                <div>
                                    <h3 className={`font-bold text-base tracking-widest ${theme.accentText}`}>DEVICE_STATUS</h3>
                                    <p className={`text-xs uppercase mt-1 ${darkMode ? 'text-slate-500' : 'text-gray-500'}`}>Monitoring Node: Active</p>
                                </div>
                                <div className={`flex items-center gap-2 border px-3 py-1.5 rounded text-xs transition-all ${darkMode ? 'bg-slate-950 border-slate-800 text-slate-400' : 'bg-gray-50 border-gray-200 text-gray-600'}`}>
                                    <div className={`w-2 h-2 rounded-full ${isSyncing ? 'bg-cyan-400 animate-ping' : 'bg-slate-500'}`}></div>
                    ...
                    <div className={`border ${theme.cardBorder} ${darkMode ? 'bg-black/40' : 'bg-white'} rounded-xl overflow-hidden backdrop-blur-sm shadow-sm`}>
                        <div className={`p-4 border-b ${theme.cardBorder} ${darkMode ? 'bg-slate-900/30' : 'bg-gray-50'} flex justify-between items-center`}>
                            <h3 className={`text-base font-bold tracking-wider ${darkMode ? 'text-slate-300' : 'text-gray-700'}`}>Activity_Logs</h3>
                            <span className={`text-xs font-mono ${darkMode ? 'text-slate-600' : 'text-gray-400'}`}>Auto-refresh 200ms</span>
                        </div>
                            ...
                            <table className="min-w-full text-sm font-mono">
                                <thead className={theme.tableHeader}>
                                    <tr>
                                        ...
                                        <th className="py-4 px-6 text-left font-bold tracking-wider">GESTURE</th>
                                        <th className="py-4 px-6 text-left font-bold tracking-wider">COMMAND</th>
                                        <th className="py-4 px-6 text-center font-bold tracking-wider">LATENCY</th>
                                        <th className="py-4 px-6 text-center font-bold tracking-wider">STATE</th>
                                    </tr>
                                </thead>
                                <tbody className={`divide-y ${darkMode ? 'divide-slate-800/50' : 'divide-gray-200'}`}>
                                    {logs.map((log, index) => (
                                        <tr key={index} className={`transition-colors duration-200 ${theme.tableRowHover} ${index === 0 && darkMode ? 'bg-cyan-950/20' : ''} ${index === 0 && !darkMode ? 'bg-blue-50' : ''}`}>
                                            {/* Fix Timestamp: Pakai helper formatTime */}
                                            <td className={`py-4 px-6 ${theme.tableText}`}>{formatTime(log.timestamp)}</td>
                                            
                                            <td className={`py-4 px-6 font-bold ${darkMode ? 'text-cyan-300' : 'text-blue-700'}`}>{log.target_gesture}</td>
                                            <td className="py-4 px-6">
                                                <span className={`inline-block px-2 py-1 rounded text-xs border font-bold ${
                                            ...
                                            <td className={`py-4 px-6 text-center ${darkMode ? 'text-slate-500' : 'text-gray-500'}`}>{parseFloat(log.wifi_latency_ms).toFixed(0)}ms</td>
